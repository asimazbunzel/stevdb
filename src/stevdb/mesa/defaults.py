...
from ..io.io import parse_fortran_value_to_python
...
def get_mesa_defaults(mesa_dir: Union[str, Path] = "") -> dict:
    ...
    if mesa_dir == "":
        if os.environ.get("MESA_DIR") is None:
            raise ValueError("`mesa_dir` cannot be empty. also it was not find in the environment variable list")
    ...
    if "bin2dco" in str(mesa_dir):
        fname = mesa_dir / "src/bin2dco_controls.defaults"
