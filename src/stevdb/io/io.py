...
def dump_dict_to_namelist_string(data: dict = {}, namelist: str = "", array_inline: bool = False) -> str:
    ...
    for key, value in data.items():
        if isinstance(value, list):
            if array_inline:
                lines.append(f"   {key} = {''.join([parse_python_value_to_fortran(value=v) for v in value])}")
            else:
